  ...
  */
  class Datatables
  {
    ...
    */
    ...
    public function having($expresion)
    {
      $this->having = $expresion;
      $this->ci->db->having($expresion);
      return $this;
    }
    ...
    * @return mixed
    */
    public function where($key_condition, $val = NULL, $backtick_protect = FALSE)
    {
    ...
    * @return mixed
    */
    public function or_where($key_condition, $val = NULL, $backtick_protect = FALSE)
    {
      $this->where[] = array($key_condition, $val, $backtick_protect);
    ...
    * @return mixed
    */
    public function like($key_condition, $val = NULL, $backtick_protect = FALSE)
    {
      $this->where[] = array($key_condition, $val, $backtick_protect);
    ...
    * @return mixed
    */
    public function filter($key_condition, $val = NULL, $backtick_protect = FALSE)
    {
   ...
   public function set_order($column,$sort){
      $this->ci->db->order_by($column,$sort);
      return $this;
   }
   public function unset_order($columns)
    {
      foreach($this->explode(',', $columns) as $val)
      {
        $column = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $val));
        $this->columns[] =  $column;
        $this->unset_order[$column] =  trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$1', $val));
      }
      return $this;
    }
    ...
    */
    protected function get_filtering()
    {
      ...
      $sWhere = '';
      $sSearch = pg_escape_string($this->ci->input->post('sSearch'));
      $mColArray = array_values(array_diff($mColArray, $this->unset_columns));
      $columns = array_values(array_diff($this->columns, $this->unset_columns));
      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
          if($this->ci->input->post('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns)){
            if(count(@$this->unset_order)>0){
              if(!in_array($this->select[$mColArray[$i]], $this->unset_order)){
                $sWhere .= $this->select[$mColArray[$i]] . " ILIKE '%" . $sSearch . "%' OR ";
              }
            }else{
                $sWhere .= $this->select[$mColArray[$i]] . " ILIKE '%" . $sSearch . "%' OR ";
            }
